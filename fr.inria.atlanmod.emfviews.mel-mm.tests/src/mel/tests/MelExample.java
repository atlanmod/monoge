		...
		if (args.length == 0) {
			...
			try {
			    ...
			    Resource resource = resSet.getResource(URI
			    		.createURI("models/extension1.xmi"), true);
			   
			    Model mo = (Model) resource.getContents().get(0);
			    List<EObject> extensions =  getFilteredContents(mo, mmPackage.eINSTANCE.getExtension());
			    for (EObject s : extensions){
			    	EClass eClass = s.eClass();
			    	System.out.println("extension: "+s.toString());
