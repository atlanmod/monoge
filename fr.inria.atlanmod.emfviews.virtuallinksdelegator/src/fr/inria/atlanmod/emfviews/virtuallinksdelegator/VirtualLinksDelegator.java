...
public class VirtualLinksDelegator {
	...
	public VirtualLinksDelegator(String linksDslFile) throws CoreException {
		System.out.println("virtuallinksdelegator virtuallinksdelegator");
		this.linksDslFile = linksDslFile;
		String dslExtension = linksDslFile.substring(linksDslFile
				.lastIndexOf('.') + 1);
		...
		for (int i = 0; i < extensions.length && !finished; i++) {
			...
			for (int j = 0; j < configElements.length && !finished; j++) {
				if (configElements[j].getAttribute("fileExtension")
					...
					if (Boolean.parseBoolean(configElements[j]
	...
	public void createVirtualMetamodelLinks(URI virtualLinksModelURI)
		...
		System.out.println("virtuallinksdelegator createvirtualmetamodellinks");
		try {
	...
	public void createVirtualModelLinks(URI linksModelURI,
		...
		System.out.println("virtuallinksdelegator createvirtualmetamodellinks 2");
		virtualLinksDelegate.createVirtualModelLinks(linksDslFile,
				linksModelURI, contributingModels);
	}
